...
# Opens a TCP server on 127.0.0.1:6000. A single client (the Tk UI) can connect,
# request DOF info, push joint targets, and receive telemetry.
try:
...
ROBOT_PATH = None
HOST, PORT = "0.0.0.0", 6000     # ✅ מאזין לכל הממשקים, כולל VM
TELEMETRY_HZ = 20
# ----------------------------
...
def main():
    ...
    # Target buffer (what the sim will apply each frame)
    ...
    try:
        ...
        while simulation_app.is_running():
                    ...
                    try:
                        ...
                        if not chunk:
                            ...
                            while b"\n" in client_buf:
                                ...
                                # Commands:
                                # {"cmd":"hello"} -> reply with dof metadata
                                # {"cmd":"set", "pos":{"JointName": rad, ...}}
                                # {"cmd":"set_idx","q":[...]}  (full array)
                                ...
                                # Optional: immediate echo of state
                                # send_json({"event":"ack"})
            ...
            # Telemetry to client at TELEMETRY_HZ
            now = time.time()
