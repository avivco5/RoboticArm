...
FEEDBACK_FMT = 'legacy16'  # 'legacy16' (pos=int16, vel=int16, tau=int8) | 'nibble12' (pos=int16, vel/tau=12bit)
...
# Position triangle
POS_AMP       = 0.5    # ± amplitude in rad
KP_CMD        = 20.0   # position gains (required in position mode)
KD_CMD        = 1.0
POS_PERIOD_S  = 4.0
POS_CYCLES    = 2
# ----------------------------------------------------
...
def decode_feedback_legacy16(payload: bytes):
    """
    Format you reported works:
      Byte0: ID
      Byte1-2: pos int16 (BE) → [-12.5..+12.5] rad
      Byte3-4: vel int16 (BE) → [-65..+65] rad/s
      Byte5:   tau int8  → [-18..+18] Nm
    """
    ...
    pos_rad = (pos_raw / 32767.0) * P_MAX

    # vel
    vel_raw = (payload[3] << 8) | payload[4]
    if vel_raw & 0x8000: vel_raw -= 0x10000
    vel_rs = (vel_raw / 32767.0) * V_MAX

    # tau
    tau_raw = payload[5]
    if tau_raw & 0x80: tau_raw -= 0x100
    tau_nm = (tau_raw / 127.0) * T_MIN * (-1) if T_MIN == -18.0 else (tau_raw / 127.0) * 18.0
    # פשטנו: אם T_MIN=-18 ו-T_MAX=18, אז:
    tau_nm = (tau_raw / 127.0) * 18.0

    return drv_id, pos_rad, vel_rs, tau_nm, pos_raw, vel_raw, tau_raw

def decode_feedback_nibble12(payload: bytes):
    """
    Doc format:
      Byte0: ID
      Byte1-2: pos int16 (BE)
...
def decode_feedback(payload: bytes):
    if FEEDBACK_FMT == 'legacy16':
        return decode_feedback_legacy16(payload)
    else:
        return decode_feedback_nibble12(payload)
...
def recv_thread(bus):
    ...
    if new:
        w.writerow(['ts_ms','can_id','dlc','raw_hex',
                    ...
                    'raw_pos','raw_vel_or_s12','raw_tau_or_s12','fmt'])
    try:
        while True:
            msg = bus.recv(timeout=1.0)
            if not msg:  continue
            if RX_ID is not None and msg.arbitration_id != RX_ID:  continue
            ...
            if dec:
                drv_id, pos, vel, tau, raw_a, raw_b, raw_c = dec
                print(f"    Decoded[{FEEDBACK_FMT}]: id={drv_id} | pos={pos:.3f} rad | vel={vel:.3f} rad/s | tau={tau:.3f} Nm")
            else:
                drv_id = pos = vel = tau = raw_a = raw_b = raw_c = None
            ...
            w.writerow([ts_ms, f"0x{msg.arbitration_id:03X}", msg.dlc, raw_hex,
                        ...
                        raw_a if raw_a is not None else "",
                        raw_b if raw_b is not None else "",
                        raw_c if raw_c is not None else "",
                        FEEDBACK_FMT])
            f.flush()
    finally:
        f.close()
...
def main():
    print(f"Mode test on {CHANNEL}: TX=0x{TX_ID:02X}, RX=0x{RX_ID:02X}, MODE={MODE}, RATE={RATE_HZ}Hz, FMT={FEEDBACK_FMT}")
    bus = open_bus()

    # Enable and set zero (helps to get coherent position)
    send_enable(bus); time.sleep(0.05)
    set_zero_point(bus); time.sleep(0.05)
